...
public abstract class SpellDictionaryASpell implements SpellDictionary {
    ...
    public List<Word> getSuggestions(String word, int threshold, int[][] matrix) {
        ...
        List<Word> result = new ArrayList<Word>(phoneticList.size());
        for (Word w : phoneticList)
            if (EditDistance.getDistance(word, w.getWord(), matrix) <= threshold)
                result.add(w);
     ...
     * @see #addBestGuess(String, java.util.List, int[][])
     */
     ...
     *
     * @param word     - the word we are trying spell correct
     * @param wordList - the  list that will contain the best guesses
     * @param matrix      dimensional array of int used to calculate
     *                 edit distance. Allocating this memory outside of the function will greatly improve efficiency.
     */
