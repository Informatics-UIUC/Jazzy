...
public class SpellDictionaryHashMap extends SpellDictionaryASpell {
    ...
    /**
     * Field indicating what the prefix is for specifying comments in the dictionary file.
     * Any line in the dictionary file that starts with this prefix will be considered a comment and ignored
     */
     ...
     */
    protected Map<String, List<String>> mainDictionary = new Hashtable<String, List<String>>(INITIAL_CAPACITY);
    /**
     ...
     *                             file
     */
    public SpellDictionaryHashMap(String name, Reader wordList) throws IOException {
        super((File) null);
        createDictionary(new BufferedReader(wordList));
        setName(name);
    }
    ...
    public SpellDictionaryHashMap(File wordList) throws IOException {
        ...
        dictFile = wordList;
        String dictFileName = wordList.getName();
        setName(dictFileName.substring(0, dictFileName.lastIndexOf(".")));
    }
     ...
     *                             file
     */
    public SpellDictionaryHashMap(String name, File wordList) throws IOException {
        this(new FileReader(wordList));
        dictFile = wordList;
        setName(name);
    }
    ...
    public void addWord(String word) {
        ...
        FileWriter w = null;
        try {
            w = new FileWriter(dictFile.toString(), true);
            // Open with append.
            w.write(word);
            w.write("\n");
        }
    ...
    public boolean isCorrect(String word) {
        ...
        if (possible.contains(word))
            return true;
            //JMH should we always try the lowercase version. If we don't then capitalised
